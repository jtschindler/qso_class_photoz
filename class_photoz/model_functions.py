...
def create_simple_bins(model_catalog,bin_size):
    ...
    bins = np.append(bins,[5,6],bin_size_highest)
    ...
    return redshift_bins, bin_data



def create_minimum_obj_bins(model_catalog,objects_per_bin, z_column_name):
    """ This function prepares the model catalog to be binned by

        Parameters:
            model_catalog : dataframe
            Dataframe containing the features

            objects_per_bin : integer
            Maximum number of objects per bin

            z_column_name : string
        ...
        Returns:
            ...
            redshift and the redshift width in this order.
        """
    #Calculate bins with at least X objects in them

    # copy redshift columns from dataframe DataFrame
    redshifts = model_catalog[[z_column_name]]

    redshifts = redshifts.sort_values(z_column_name)
