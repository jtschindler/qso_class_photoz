...
def dr7dr12q_grid_search():
    ...
    df_stars = pd.read_hdf('../class_photoz/data/DR13_stars_ps_clean_flux_cat.hdf5','data')
    df_quasars = pd.read_hdf('../class_photoz/data/DR7DR12Q_ps_clean_flux_cat.hdf5','data')
    df_stars.dropna(subset=['star_class'],inplace=True)

    param_grid = [{'n_estimators': [200,300,400], 'min_samples_split': [2,3,4],
                    'max_depth' : [15,20,25]}]
    # param_grid = [{'n_estimators': [50], 'min_samples_split': [2],
    #                 'max_depth' : [20]}]
    rand_state=1
    scores = ['f1_weighted']
    ...
    df_train, df_pred = qs.make_train_pred_set(df_stars_train, df_qsos_train, 0.2 ,rand_state, 'i21_5_')
    ...
    rf_class.rf_class_grid_search(df_train, df_pred, features, label ,param_grid, rand_state, scores,'PS_i215')
    ...
    rf_class.rf_class_grid_search(df_train, df_pred, features, label ,param_grid, rand_state, scores, 'PSW1W2_i215')
    ...
    features = ['PS_i', 'WISE_w1', 'TMASS_j', 'gr', 'ri', 'iz', 'zy', 'yj','jh',  \
                'hk', 'kw1', 'w1w2']
    ...
    rf_class.rf_class_grid_search(df_train, df_pred, features, label ,param_grid, rand_state, scores, 'PSTMASSW1W2_i215')
...
def full_test():
    ...
    df_stars.query('iMeanPSFMag <= 21.5',inplace=True)
    df_quasars.query('iMeanPSFMag <= 21.5',inplace=True)
    ...
    df.to_hdf('ps_fitted_classes.hdf5','data')
